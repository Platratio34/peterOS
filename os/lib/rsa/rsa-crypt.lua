...
local function encrypt(text, publicKey)
	local maxLen = 256 / 8
	if text:len() <= maxLen then
		local res = bytesToNumber(stringToBytes(msg), 256, 8)
		return crypt(publicKey, res)
    end
	local out = ""
	for i=1,math.ceil((text:len())/maxLen) do
		local str = text:sub((i-1)*maxLen,i*maxLen)
        local res = bytesToNumber(stringToBytes(str), 256, 8)
		out = out..crypt(publicKey, res)
    end
	return out
end

local function decrypt(text, privateKey)
	
end

return { bytesToNumber = bytesToNumber, crypt = crypt, numberToBytes = numberToBytes, bytesToString = bytesToString, stringToBytes = stringToBytes, encrypt = encrypt }
